...
public class ListTables {
	public static String listTable(JsonObject conn_obj)
		...
		if (system.equals("Flatfiles")) {
			String filepath = conn_obj.get("url").getAsString();
			...
			// Connection con =
			// DriverManager.getConnection("jdbc:SQLServer://localhost:1433;databaseName=DEMO_DB",properties0);
			...
			if (system.equals("MySQL")) {
				rs1 = stmt0.executeQuery(
						"SELECT a.TABLE_NAME,group_concat(b.COLUMN_NAME SEPARATOR'-') as PrimaryKey FROM INFORMATION_SCHEMA.TABLES a LEFT JOIN INFORMATION_SCHEMA.COLUMNS b ON a.TABLE_NAME = b.TABLE_NAME AND b.COLUMN_KEY = 'PRI' AND a.TABLE_SCHEMA = b.TABLE_SCHEMA  WHERE a.TABLE_SCHEMA = '"
								+ conn_obj.get("schema").getAsString() + "' GROUP BY a.TABLE_NAME;");
				...
				rs1 = stmt0.executeQuery(
						"SELECT DISTINCT TableName as TABLE_NAME,'NoPrimaryKey' as PrimaryKey FROM DBC.ColumnsV WHERE DatabaseName = '"
								+ conn_obj.get("database").getAsString() + "';");
			} else if (system.equals("Teradata")) {
				rs1 = stmt0.executeQuery(
						"SELECT TRIM(T.TableName) AS TABLE_NAME, I.Primary_Key AS PrimaryKey FROM DBC.TablesV T LEFT JOIN (SELECT TABLENAME, TRIM(TRAILING '-' FROM (XMLAGG(TRIM(Columnname)|| '-' ) (VARCHAR(10000)))) as Primary_Key FROM dbc.indicesV WHERE indextype = 'K' GROUP BY 1) I ON I.TableName = T.TableName WHERE T.databasename = '"
								+ conn_obj.get("database").getAsString() + "';");
			} else if (system.equals("Oracle")) {
				System.out.println("inside first query loop" + conn_obj.get("schema").getAsString());
				rs1 = stmt0.executeQuery(
						"SELECT cols.TABLE_NAME, LISTAGG(cols.column_name,'-') WITHIN GROUP (ORDER BY cols.column_name)as \"PrimaryKey\" FROM all_constraints cons, all_cons_columns cols WHERE cols.OWNER = '"
								+ conn_obj.get("schema").getAsString()
								+ "' AND cons.constraint_type = 'P' AND cons.constraint_name = cols.constraint_name AND cons.owner = cols.owner GROUP BY cols.TABLE_NAME");
			}
			...
			while (rs1.next()) {
				...
				// ResultSet rs2 = null;
				// if(system.equals("MySQL")) {rs2 = stmt1.executeQuery("SELECT COLUMN_NAME FROM
				// INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA =
				// '"+migrationProcessDTO.getSourceConnectionSchema()+"' AND TABLE_NAME =
				// '"+rs1.getString("TABLE_NAME")+"';");}
				// else if(system.equals("SQLServer")) {rs2 = stmt1.executeQuery("SELECT
				// COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_CATALOG =
				// '"+migrationProcessDTO.getSourceConnectionDatabase()+"' AND TABLE_NAME =
				// '"+rs1.getString("TABLE_NAME")+"';");}
				// else if(system.equals("Netezza")) {rs2 = stmt1.executeQuery("SELECT
				// COLUMN_NAME FROM _V_SYS_COLUMNS WHERE TABLE_SCHEMA =
				// '"+migrationProcessDTO.getSourceConnectionSchema()+"' AND TABLE_NAME =
				// '"+rs1.getString("TABLE_NAME")+"';");}
				// else if(system.equals("Teradata")) {rs2 = stmt1.executeQuery("SELECT
				// TRIM(ColumnName) as COLUMN_NAME FROM DBC.Columns V WHERE DatabaseName =
				// '"+migrationProcessDTO.getSourceConnectionDatabase()+"' AND TableName =
				// '"+rs1.getString("TABLE_NAME")+"';");}
				// else if(system.equals("Oracle")) {System.out.println("inside loop2 oracle
				// if");
				// rs2 = stmt1.executeQuery("SELECT Column_Name as COLUMN_NAME FROM
				...
				// //ResultSet rs3 = stmt2.executeQuery("SELECT COLUMN_NAME FROM
				// ALL_TAB_COLUMNS");
				// ResultSet rs3 = null;
				...
				// INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA =
				// '"+migrationProcessDTO.getSourceConnectionSchema()+"' AND TABLE_NAME =
				// '"+rs1.getString("TABLE_NAME")+"' AND DATA_TYPE IN
				// ('timestamp','datetime');");}
