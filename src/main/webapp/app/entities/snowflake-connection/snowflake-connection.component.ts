...
@Component({
  selector: 'jhi-snowflake-connection',
  templateUrl: './snowflake-connection.component.html'
})
export class SnowflakeConnectionComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  loadAll() {
    this.snowflakeConnectionService
      ...
      .subscribe(
        (res: HttpResponse<ISnowflakeConnection[]>) => this.paginateSnowflakeConnections(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/snowflake-connection'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  ngOnInit() {
    ...
    this.registerChangeInSnowflakeConnections();
  }
  ...
  trackId(index: number, item: ISnowflakeConnection) {
    return item.id;
  }

  registerChangeInSnowflakeConnections() {
    this.eventSubscriber = this.eventManager.subscribe('snowflakeConnectionListModification', response => this.loadAll());
  }
  ...
  protected paginateSnowflakeConnections(data: ISnowflakeConnection[], headers: HttpHeaders) {
    ...
    this.snowflakeConnections = data;
  }
