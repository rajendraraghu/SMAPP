...
@Component({
  selector: 'jhi-source-connection',
  templateUrl: './source-connection.component.html'
})
export class SourceConnectionComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  loadAll() {
    this.sourceConnectionService
      ...
      .subscribe(
        (res: HttpResponse<ISourceConnection[]>) => this.paginateSourceConnections(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/source-connection'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  ngOnInit() {
    ...
    this.registerChangeInSourceConnections();
  }
  ...
  trackId(index: number, item: ISourceConnection) {
    return item.id;
  }

  registerChangeInSourceConnections() {
    this.eventSubscriber = this.eventManager.subscribe('sourceConnectionListModification', response => this.loadAll());
  }
